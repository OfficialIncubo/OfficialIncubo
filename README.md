<h1 align="center">
  Welcome to my profile! Thanks for visiting! ;)
</h1>

[![My ](https://readme-typing-svg.demolab.com?font=Ubuntu&size=25&duration=6500&pause=1500&color=20AEFF&center=true&vCenter=true&width=1000&lines=Hey!+I'm+Incubo_!;A+guy+that+produces+some+great+standalone+music+visualizations...;...for+DJs+and+VJs!;...and+some+experiments+and+stuff+(possibly).;I'm+also+a+2nd+year+student...;...at+Faculty+of+Automatic+Control+and+Computer+Engineering+Ia%C8%99i)](https://git.io/typing-svg)

# Introduction

Hi! I'm Incubo_ (known as Se7enSlasher). My real name is Rotaru Alexandru-Ionuț, a 22 year old with disability (confirmed with Asperger) and an IQ above average. I'm a 2nd year student at Faculty of Automatic Control and Computer Engineering Iași. I'm also developing/producing high-quality standalone music visualizations for DJs, VJs, music listeners and everyone! I'm also fan of cartoons and such; some things that even not enumarated in, please see my personal site I've wrote few years ago http://officialincubo.carrd.co/ (may contain unsuitable infos, possibly).

# What I'm actively working on my open-source projects?

Well, some two of my beloved and fully-working standalone music visualizers I've been working for 4 years. Both of them are made for DJs, VJs, music listeners and such. Here's what I'm saying:

- **[BeatDrop Music Visualizer](https://github.com/OfficialIncubo/BeatDrop-Music-Visualizer)**: It's a standalone music visualization powered by a custom [MilkDrop](http://www.geisswerks.com/milkdrop/) engine, originally built as a plug-in for a popular music player called [Winamp](https://winamp.com/) for Windows. It features an expanded animated sprite/texture support (GIF, Video and Spout Input), FFT/Wave audio data shader functions, System Time/Date variables, extended custom shapes/waves limit, live speaker/microphone audio input, customized audio processing code and so much more! Made by contributors and me.

- **[fische music visualization for Windows](https://github.com/OfficialIncubo/fische-windows)**: A Windows port version of an original Linux-based music visualizer under the same name, [fische](https://web.archive.org/web/20200801073714/http://26elf.at/musicviz/) ([source code](https://github.com/maysl/fische)). It provides much and much features, maintaining the same functionality: [Spout](https://spout.zeal.co/) output for visual sending, adjustable audio sensitivity, settings dialog, ASIO support, buttery smooth visuals and more; packed in just ONE executable. Made by a sole developer, almost mostly vibecoded with hard-tweaks and testing with trial and error.

Some note: both of these products requires Windows OS machine with necessary requirements, such as DirectX and runtime packages. As I'm a Windows guy, I had much experience on it.
Due to my other personal work or non-allocated time, the active development will maintain slower and a gradual-pace. Whatever, if I'm having a full energy, it will move (a little bit faster).

# Additional infos

- Age: 22
- Gender: Male
- Pronouns: he/him
- Former hobbies: logo editing, video editing, piano player, TV ident capturer etc.
- Actual hobbies: DJ, VJ, Music Visualization Fan, Obsessed with Cartoons, Tech, Sports (sometimes).

# Follow me on socials:

| Platform | Handle / Link | Notes |
| :--- | :--- | :--- |
| **YouTube** | [Incubo_](https://www.youtube.com/channel/UCZaNnAoBSr5KTd3d1Z_WkNQ) | Main Channel (mostly inactive due to lack of motivation or zero effort) |
| **YouTube** | [DJ XcitMosphere](https://www.youtube.com/channel/UCfHLcpgxq1K8yNuAb3W2cXA) | DJ Channel with eye-catching visuals |
| **YouTube** | [Old YT Channel, archive purpose](https://www.youtube.com/@rotarumihaitehobjectthingyvgcp) | My old and archived channel; VISIT AT YOUR OWN RISK! |
| **Twitter** | [@OfficialIncubo_](https://twitter.com/OfficialIncubo_) | Main Twitter/X Account |
| **Twitter** | [@OctobellaMilkD](https://twitter.com/OctobellaMilkD) | MilkDrop Preset Showcase Account |
| **Twitter** | [@art_by_incubo_](https://twitter.com/art_by_incubo_) | Art Account (VERY inactive) |
| **Instagram** | [officialincubo_](https://instagram.com/officialincubo_) | |
| **TikTok** | [@officialincubo_](https://tiktok.com/@officialincubo_) | |
| **Discord** | `damilkdropvisobsessor` | Main Discord |
| **Discord** | `Incubo_ (Backup)#0506` | Backup Discord |
| **Mushroom** | [@se7enslasher](https://mushroom.gg/se7enslasher) | |
| **ROBLOX** | [@TheDeerSquadFan923](https://www.roblox.com/users/88025093/profile) | |
| **GitHub** | [OfficialIncubo](https://github.com/OfficialIncubo) | |
| **YTPMania (discontinued)**| [Incubo](https://ytpmania.net/u/incubo) | |
| **Winamp Forum**| [Incubo_](https://forums.winamp.com/member/576741-incubo_) | It really whips the Llama's Ass! |

# Random jokes, hah.

![Jokes Card](https://readme-jokes.vercel.app/api)
